...
async function login(req, res, next) {
    try {
        ...
        context.query("select * from users where username=?;", [req.body.userName], function (err1, users) {
            if (err1) throw err1;
            ...
            if (users.length > 1) {
                return res.status(400).json("More than one user with the same username. Please contact the administrator.")
            }

            const user = users[0]

            if (user.NumberOfAttempts > 10) {
                return res.status(400).json("You have reached the max number of attempts to login.")
            }
            ...
            if (user.Password != req.body.password) {
                ...
                context.query("update users set NumberOfAttempts=? where id=?", [numberOfAttempts, user.Id], function (err2) {
                    if (err2) throw err2;
                })
                return res.json("Failed attempt to login. Attempts left=" + numberOfAttempts)
            }

            if (user.NumberOfAttempts > 0) {
                context.query("update users set NumberOfAttempts=? where id=?", [0, user.Id], function (err2) {
                    if (err2) throw err2;
                })
            }
            context.query(`SELECT role.role FROM transam.userroles as role
            left join transam.usermappings as mapping on role.Id=mapping.userroleid
            where userid=?;`, [user.Id], function (err2, roles) {
                if (err2) throw err2;
                //console.log("roles", roles)
                const rolesString=roles.map(c=>c.role).join(",")
                console.log("rolesString",rolesString )
                
                let timeStamp = Math.floor(Date.now() / 1000) + (60 * 60 * settings.JWTdurationInHours);
